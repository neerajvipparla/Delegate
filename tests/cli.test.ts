...
import { buildOpencodeArgs } from "../src/opencode/cli.js";

test("builds minimal args for a bare prompt", () => {
  const args = buildOpencodeArgs({ prompt: "hello", workingDirectory: "/tmp/proj" });
...
test("puts -- immediately before the prompt even with a leading dash", () => {
  const args = buildOpencodeArgs({ prompt: "-1 plus 1", workingDirectory: "/tmp/proj" });
  assert.equal(args.at(-2), "--");
  assert.equal(args.at(-1), "-1 plus 1");
});

test("adds -s for session continuation", () => {
  const args = buildOpencodeArgs({ prompt: "hi", workingDirectory: "/tmp/proj", sessionId: "ses_abc" });
...
test("ignores fork when no session id is present", () => {
  const args = buildOpencodeArgs({ prompt: "hi", workingDirectory: "/tmp/proj", fork: true });
  assert.ok(!args.includes("--fork"));
  assert.ok(!args.includes("-s"));
});
